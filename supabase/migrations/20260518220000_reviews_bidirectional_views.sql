-- Update client_booking_details: add tasker_user_id and filter review to client-only (is_reviewer_tasker = false)
DROP VIEW IF EXISTS public.client_booking_details;
CREATE VIEW public.client_booking_details AS
SELECT
  b.id,
  b.client_id,
  b.status,
...
LEFT JOIN LATERAL (
  SELECT p.id, p.amount, p.service_comission, p.subtotal, p.discount, p.total_amount, p.payment_card_type, p.payment_card_last_four
  FROM booking_payments p
  WHERE p.booking_id = b.id
  ORDER BY p.created_at DESC
  LIMIT 1
) pay ON true;
-- Grant access to authenticated users (same as before)
GRANT SELECT ON public.client_booking_details TO authenticated;
-- Update tasker_booking_details: filter review to tasker-only (is_reviewer_tasker = true)
CREATE OR REPLACE VIEW public.tasker_booking_details AS
WITH client_reviews AS (
  SELECT r.reviewed_user_id AS client_id,
...
LEFT JOIN LATERAL (
  SELECT p.id, p.booking_id, p.amount, p.service_comission, p.subtotal, p.discount, p.total_amount, p.payment_card_type, p.payment_card_last_four, p.status, p.created_at
  FROM booking_payments p
  WHERE p.booking_id = b.id
  ORDER BY p.created_at DESC
  LIMIT 1
